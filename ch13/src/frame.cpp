...
namespace myslam {
    ...
    void Frame::SetKeyFrame() {                 //设置关键帧：设置静态ID（第一次时设置即可），is_keyframe = true, ID++（关键帧计数）
    ...
    void Frame::SetRawFrame(){
        static long raw_frame_factory_id = 0;
        raw_frame_id_ = raw_frame_factory_id++;
    }
}