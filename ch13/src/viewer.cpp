...
namespace myslam {
    ...
    void Viewer::UpdateMap() {                                      //Viewer::更新地图
        std::unique_lock<std::mutex> lck(viewer_data_mutex_);
        assert(map_ != nullptr);                                    //判断map是否为空
        active_keyframes_ = map_->GetActiveKeyFrames();             //把窗口关键帧
        active_landmarks_ = map_->GetActiveMapPoints();             //和窗口路标点 添加到可视化函数中
        map_updated_ = true;
    }
