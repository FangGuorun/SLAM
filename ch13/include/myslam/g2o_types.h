...
/// 带有地图和位姿的二元边
    ...
    public:
        ...
        virtual void computeError() override {
            const VertexPose *v0 = static_cast<VertexPose *>(_vertices[0]); //pose顶点
            const VertexXYZ *v1 = static_cast<VertexXYZ *>(_vertices[1]);   //landmark顶点
            SE3 T = v0->estimate();                                     //v0->estimate:v0顶点优化的是关键帧的位姿
            Vec3 pos_pixel = _K * (_cam_ext * (T * v1->estimate()));    //v1->estimate:v1顶点优化的是路标点的世界坐标系坐标
            pos_pixel /= pos_pixel[2];
            _error = _measurement - pos_pixel.head<2>();
        }
